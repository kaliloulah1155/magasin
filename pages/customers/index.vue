    ...
    <template v-if="accessRights.canView">
      <v-banner lines="one" color="warning">
        <template v-slot:text>
          <h1 class="text-grey mb-4">Clients potentiels</h1>
        </template>
        <template v-if="accessRights.canCreate" v-slot:actions>
          <popup-client
            @saveItem="getItem"
            :profils="profils"
            :item_statut="item_statut"
      ...
      <v-container fluid class="my-5">
        <v-card flat>
          <v-card-title class="d-flex align-center pe-2">
            <v-icon icon="view_list"></v-icon>&nbsp; Liste des clients
            <v-spacer></v-spacer>
            <v-text-field
              v-model="search"
          ...
          <v-data-table
            ...
            :headers="headers"
            :items="clients"
            :loading="clientLength > 0 ? false : true"
          ...
          >
            ...
            <template v-slot:item.lib_active="{ item }">
              <v-chip
                :color="item.lib_active === 'Activé' ? 'green' : 'red'"
                :value="item.lib_active"
...
export default {
  setup() {
    ...
    return { authStore, clientStore, token, accessRights };
  },
  data: () => ({
    dialog: false,
    dialogDelete: false,
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        if (
          this.editedItem.profil_id &&
          this.editedItem.profil_id.hasOwnProperty("libelle")
        ) {
        ...
        if (
          ...
          this.editedItem.statut.hasOwnProperty("libelle")
        ) {
          updatedObject.statut = this.editedItem.statut.id;
        }

        if (
          this.editedItem.lib_sexe &&
          this.editedItem.lib_sexe.hasOwnProperty("libelle")
        ) {
    ...
    getItem(fromPopup) {
      ...
      if (
        fromPopup.profil_id &&
        fromPopup.profil_id.hasOwnProperty("libelle")
      ) {
